...
/// Copy the `lilypond/` data directory into the build target
...
fn copy_data_dirs(manifest: &str) {
    ...
    // In release builds, lilypond/ is downloaded at runtime by the updater.
    // Only bundle it for local debug builds.
    if profile == "debug" {
        let src_dir = manifest.join("lilypond");
        if src_dir.exists() {
            copy_dir_recursive(&src_dir, &target_dir.join("lilypond"));
        }
    }
}
